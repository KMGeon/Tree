...
public class T08ServletFilter implements Filter {
	/*
	 * 서블릿 필터에 대하여... 1. 사용목적 - 클라이언트의 요청을 수행하기 전에 가로채서 필요한 작업을 수행할 수 있다. - 클라이언트에
	 * 응답정보를 제공하기 전에 응답정보에 필요한 작업을 수행할 수 있다.
	 * 
	 * 2. 사용 예 - 인증필터 - 데이터 압축필터 - 인코딩 필터 - 로깅 및 감사처리 필터 - 이미지 변환 필터 등.
	 */
	@Override
	public void destroy() {
		// 펄터객체가 컨테이너에 의해 서비스로부터 제거되기 전에 호출됨.

...
//		ProductVO [productId=P1236, pname=null, unitPrice=0, description=null, 
	...
	@PostMapping("/addCart")
	public String addCart(@RequestParam String productId, Model m, @ModelAttribute ProductVO productVO,
		...
		for(int i=0;i<list.size();i++) {
			if(list.get(i).getProductId().equals(productId)) {
				cnt++;
				list.get(i).setQuantity(list.get(i).getQuantity()+1);//장바구니에 상품이 이미 있으면 개수만 1 증가
			}
		}
		if(cnt==0) {//장바구니에 상품이 없다면
			vo.setQuantity(1);//최종목표 : 장바구니에 상품을 추가
			list.add(vo);
		}

...
<%@page import="kr.or.funding.wishlist.vo.WishlistVO"%>
	...
	<div id="wrap">
		<div id="header">
			<div class="user_menu">
				...
				<ul class="sign_menu">
					<!--login class 추가-->
					<li class="link"><a href="../accounts/join.html"
						class="item after join">회원가입</a></li>
					<li class="link"><a href="../accounts/login.html"
						class="item after login_none">로그인</a> <a
						class="item login login_check grade_comm"> <span
							class="ico_grade grade0">일반</span> <span class="txt"> <span
								class="name"></span> <span class="sir">님</span>
						</span>
					</a>
						<ul class="sub">
							<li class="list"><a href="#" class="list_item">주문 내역</a></li>
							<li class="list"><a href="#" class="list_item">배송지 관리</a></li>
							<li class="list"><a href="#" class="list_item">개인 정보 수정</a>
							</li>
	...
	<div
		...
		<table class="notice_style" style="width: 100%; height: 600px">
			...
			<tbody>
				...
				%>
				<tr>
<!-- 찜목록번호 -->	<td style="text-align: center"><%=i%></td>
<!-- 상품번호 -->	<td style="text-align: center"><%=wishList.get(i).getPtNum()%></td>	
<!-- 회원번호 -->	<td ><%=wishList.get(i).getMbsNum()%></td>			
<!-- 상품이름 -->	<td style="text-align: center"><%=wishList.get(i).getPtNm() %></td>			
<!-- 소개내용 -->	<td style="width: 20%"><%=wishList.get(i).getItdt() %></td>			
<!-- 가격 -->		<td style="text-align: center"><%=wishList.get(i).getPtPrc() %></td>
<!-- 시작기간 -->	<td style="text-align: center"><%=wishList.get(i).getStDt() %></td>
<!-- 마감기간 -->	<td style="text-align: center"><%=wishList.get(i).getEdDt() %></td>
<!-- 수량 -->		<td style="text-align: center"><%=wishList.get(i).getPyQty() %></td>
<!-- 카테고리 -->	<td style="text-align: center"><%=wishList.get(i).getPtCgy() %></td>
				<td style="font-size: 12px"><%=simpleDateFormat.format(wishList.get(i))%></td>
				<td>
				<input type="hidden" name="atchFileId" value="<%=wishList.get(i).getAtchFileId()%>">				
				<input type="hidden" name="mbsAhy" value="<%=wishList.get(i).getMbsAhy()%>">				
				<input type="hidden" name="mbsId" value="<%=wishList.get(i).getMbsId()%>">				
				<input type="hidden" name="mbsPw" value="<%=wishList.get(i).getMbsPw()%>">				
				<input type="hidden" name="mbsMail" value="<%=wishList.get(i).getMbsMail()%>">				
				<input type="hidden" name="mbsAddr" value="<%=wishList.get(i).getMbsAddr()%>">				
				<input type="hidden" name="brDt" value="<%=wishList.get(i).getBrDt()%>">				
				<input type="hidden" name="mbsPh" value="<%=wishList.get(i).getMbsPh()%>">				
				<input type="hidden" name="rfCd" value="<%=wishList.get(i).getRfCd()%>">				
				<input type="hidden" name="useAt" value="<%=wishList.get(i).getUseAt()%>">				
				<input type="hidden" name="milgNum" value="<%=wishList.get(i).getMilgNum()%>">				
				</td>
		...
		%>
		<a id="pagenum" href="/wishListList.do?pageNum=<%=startPage - pageBlock%>">&nbsp;Prev</a>
		<%
			}
		%>
		...
		%>
		<a id="pagenum" href="/wishListList.do?pageNum=<%=i%>"><%=i%>&nbsp;</a>
		<%
			}
		%>
		...
		<%
			if (endPage < pageCount) {
		%>
		<a id="pagenum" href="/wishListList.do?pageNum=<%=startPage + pageBlock%>">&nbsp;Next</a>
		<%
			}
		%>
		<%

...
<%@page import="kr.or.funding.board2.vo.Notice2VO"%>
<%@page import="java.util.List"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
	
<%
	List<Notice2VO> notice2List = (List<Notice2VO>) request.getAttribute("notice2List");
	SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd");
	
	int cnt = notice2List.size();
...
<body>
			...
			<div class="user_menu">
				  ...
				  <ul class="sign_menu">
                    ...
                    <li class="link">
                        ...
                        <%} else{ %>
                        <li class="link">
                        <a href="/Funding/view/joinselect.html" class="item after join"><%=memVo.getMbsNm()%>님 어서오세요</a>
                    </li>
                    ...
                    <li class="link">
                    	...
                    	<%} %>
                    <li class="link">
                        <a href="/Funding/view/logout.jsp" onclick="logout()" class="item after login_none">로그아웃
                        </a>
                    </li> 
                        <%} %>
                        </ul>
	...
	<div
		...
		<table class="notice_style" style="width: 100%; height: 600px">
			...
			<tbody>
				...
				<tr>
					...
					<td style="width: 60%"><a
						href="/Funding/notice2/notice2Detail.do?NtNum=<%=notice2List.get(i).getNtNum()%>"><%=notice2List.get(i).getNtTit()%></a></td>
					<td style="font-size: 12px"><%=simpleDateFormat.format(notice2List.get(i).getNtWriteDt())%></td>
				</tr>
			...
			<%if(memVo.getMbsAhy() == 0){ %>
				<td colspan="1">
				<input type="button" value="등록하기" class="custom-btn btn-1" onclick ="self.location='/Funding/view/board/boardInsert.jsp'">			
				</td><br>
				<%} %>
		<br>
		...
		%>
		<a id="pagenum" href="notice2List.do?pageNum=<%=startPage - pageBlock%>">&nbsp;Prev</a>
		<%
			}
		%>
		...
		%>
		<a id="pagenum" href="notice2List.do?pageNum=<%=i%>"><%=i%>&nbsp;</a>
		<%
			}
		%>
		...
		<%
			if (endPage < pageCount) {
		%>
		<a id="pagenum" href="notice2/notice2List.do?pageNum=<%=startPage + pageBlock%>">&nbsp;Next</a>
		<%
			}
		%>
		<%
